...
class TextData:
    ...
    def _createBatch(self, samples):
        ...
        pre_sort_list = [(a,b,c,d,e,f) for a,b,c,d,e,f in zip(batch.encoderSeqs, batch.encoder_lens, batch.decoderSeqs, batch.decoder_lens, batch.targetSeqs,batch.weights)]
        post_sorted_list = sorted(pre_sort_list, key=lambda x:x[1], reverse=True)
        batch.encoderSeqs = [a[0] for a in post_sorted_list]
        batch.encoder_lens = [a[1] for a in post_sorted_list]
        batch.decoderSeqs = [a[2] for a in post_sorted_list]
        batch.decoder_lens = [a[3] for a in post_sorted_list]
        batch.targetSeqs = [a[4] for a in post_sorted_list]
        batch.weights = [a[5] for a in post_sorted_list]
